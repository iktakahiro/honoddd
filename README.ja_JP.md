...
backend は local の PGlite data をデフォルトで `apps/backend/.pglite` に保存します。別の data directory を使う場合は `PGLITE_DATA_DIR` を設定します。test では明示的に `new PGlite()` を渡すことで in-memory database を使います。
...
**なにをしているか**: `apps/backend/src/bootstrap/app-container.ts` が application container を構築します。具象の Drizzle repository、transaction manager、UseCase instance はここで生成します。

**なぜそうするか**: object construction は composition の責務です。container に集約すると、repository lifecycle、migration readiness、shutdown behavior を handler や UseCase から分離できます。

**なにをしているか**: `apps/backend/src/api/app.ts` は Hono app を構築し、application container と request-scoped value を含む oRPC context を渡します。

**なぜそうするか**: Hono は HTTP adapter に留まり、oRPC handler が実際に使う context は oRPC に渡します。request ID や将来の authentication context などを、UseCase constructor を変えずに追加できます。

**なにをしているか**: `orpc-router.ts` は `context.container.todoUseCases` 経由で contract handler と UseCase を対応させ、Domain Exception を oRPC error に変換します。
...
**なにをしているか**: Drizzle schema file は `src/infrastructure/psql/drizzle/schemas` に flat に置き、生成された SQL migration は `src/infrastructure/psql/migrations` に置きます。`todo-mapper.ts` は Drizzle row と `Todo` entity を相互変換します。

**なぜそうするか**: database row は domain object ではありません。明示的な mapper によって、column name、nullable field、ORM の型が UseCase に漏れることを防ぎます。

**なにをしているか**: PGlite の初期化では、生成済みの Drizzle migration を `drizzle-orm/pglite/migrator` で適用します。

**なぜそうするか**: PGlite は PostgreSQL 互換 database として扱えるため、SQL migration を初期化の source of truth にすると、local の組み込み storage と将来の PostgreSQL deployment で同じ schema history を使えます。
...
schema file を編集したあと、Drizzle migration を生成します。

```sh
bun --cwd apps/backend run db:generate
```

local の PGlite data directory に migration を適用します。
